...
ratings = pd.read_csv('/home/juno/workspace/user_collaborative_filtering/data_reco_book/u.data', names=r_cols,  sep='\t',encoding='latin-1')
ratings = ratings.drop('timestamp', axis=1).dropna()
ratings[['user_id', 'rating']] = ratings[['user_id', 'rating']].astype(float).astype(int)
ratings['movie_id'] = ratings['movie_id'].astype(float).astype(int)
ratings = ratings[ratings.rating < 6]
ratings = ratings[ratings.rating > -1]

ratings = ratings[ratings.movie_id < 10000]
